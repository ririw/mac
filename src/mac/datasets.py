...
class MAC_JSON_Dataset(Dataset):
    ...
    def __enter__(self):
        ...
        self.images = np.memmap(
            image_handle, dtype='float32', mode='r').reshape(-1, 1024, 14, 14)
        pkl_filename = 'dataset_{}.pkl'.format(self.group_name)
        with ds_fs.open(pkl_filename, 'rb') as f:
            self.dataset_entries = pickle.load(f)
